...
onMounted(() => {
    ...
    const headbar = document.querySelector('.navbar-main');
    if (headbar) {
        const headbarHeight = headbar.offsetHeight;
        document.documentElement.style.setProperty('--headbar-height', `${ headbarHeight -18}px`);
    }
});
</script>
...
header {
    ...
    background-color: white; /* 防止内容被遮挡 */
    padding: 10px 20px;
...
.sidebar {
    ...
    max-width: 200px;
    margin-right: 20px;
    position: fixed; /* 固定左侧导航栏 */
    top: 100px; /* 与顶部导航栏的底部对齐 */
    left: 70px;
    padding-left: 30px;
